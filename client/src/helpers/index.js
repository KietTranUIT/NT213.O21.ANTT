...
export const showbookmarks = async (id) => {
  try {
    console.log(id)
    const data = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/bookmark/${id}`)
    return data;
  } catch (error) {
    return { msg: "error" }
  }
...
export const deletepost = async (postid, userid) => {
  try {
    var { data } = await axios.delete(
      `${process.env.REACT_APP_BACKEND_URL}/post?postid=${postid}&userid=${userid}`
    )
    return data;
  } catch (error) {
...
export const showmyposts = async (id) => {
  try {
    var { data } = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/user/${id}/post`
    )
    return data;
  } catch (error) {
    console.log("Error in Bookmark", error);
